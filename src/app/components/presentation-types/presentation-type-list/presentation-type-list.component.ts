...
@Component({
  selector: 'app-presentation-type-list',
  standalone: true,
  imports: [CommonModule, FormsModule, ZardSelectComponent, ZardSelectItemComponent],
  templateUrl: './presentation-type-list.component.html',
  styleUrls: ['./presentation-type-list.component.css'],
})
export class PresentationTypeListComponent {
  @Input() set types(value: PresentationType[]) {
    this.typesSignal.set(value ?? []);
  }
  @Input() isLoading = false;
  @Output() refresh = new EventEmitter<void>();
  @Output() editPresentationType = new EventEmitter<PresentationType>();
  @Output() deletePresentationType = new EventEmitter<PresentationType>();

  searchTerm = '';
  sortBy = 'code';
  sortOrder: 'asc' | 'desc' = 'asc';

  private readonly typesSignal = signal<PresentationType[]>([]);
  ...
  readonly columns: ColumnDef<PresentationType>[] = [
  ...
  readonly table = createAngularTable<PresentationType>(() => ({
  ...
  getHeaderLabel(columnId: string): string {
    const labels: Record<string, string> = {
      code: this.t('presentation_type_code'),
      name: this.t('presentation_type_name'),
  ...
  onEdit(type: PresentationType): void {
    this.editPresentationType.emit(type);
  }

  onDelete(type: PresentationType): void {
    this.deletePresentationType.emit(type);
  }
