...
export interface InventoryImportDialogData {
  onSuccess?: (result: { successful: number; skipped: number; failed: number }) => void;
}

type RowStatus = 'new' | 'exists' | 'similar' | 'error' | 'duplicate';

interface InventoryMatch { id: string; sku: string; name: string; location: string; quantity: number; }
...
interface PreviewRow {
  ...
  existingInventory?: InventoryMatch;
  similarInventory?: InventoryMatch[];
  data: {
    sku: string; name: string; description: string; location: string;
    quantity: string; unit_price: string;
    track_by_lot: string; track_by_serial: string; track_expiration: string;
    min_quantity: string; max_quantity: string;
  };
}
        ...
        @if (step === 2) {
          <div class="px-6 py-8 max-w-xl mx-auto space-y-6">
            ...
            <div class="space-y-2" [ngClass]="{'opacity-40': parseProgress < 100}">
              ...
              @if (validateProgress > 0) { <p class="text-xs text-gray-400 text-right">{{ rows.length }} {{ t('articles_verified') || 'ítems verificados' }}</p> }
            </div>
          </div>
        }
              ...
              <table class="w-full text-sm border-collapse min-w-[1100px]">
                <thead>
                  <tr class="bg-gray-50 dark:bg-gray-800 sticky top-0 z-10">
                    ...
                    <th class="px-2 py-2 text-left text-gray-600 min-w-[140px]">{{ t('name') || 'Nombre' }}</th>
                    <th class="px-2 py-2 text-left font-semibold text-red-600 min-w-[120px]">{{ t('location_code') || 'Ubicación' }} *</th>
                    <th class="px-2 py-2 text-left font-semibold text-red-600 w-24">{{ t('quantity') || 'Cantidad' }} *</th>
                ...
                <tbody>
                  @for (row of rows; track row.originalRowNum) {
                    <tr class="border-b border-gray-100 dark:border-gray-800"
                      ...
                      <td class="px-1 py-1"><input type="text" [(ngModel)]="row.data.name" [disabled]="row.skip" class="w-full px-2 py-1 text-xs border border-gray-200 rounded dark:bg-gray-900 dark:border-gray-600" /></td>
                      <td class="px-1 py-1">
                        <input type="text" [(ngModel)]="row.data.location" (input)="validateRow(row)" [disabled]="row.skip"
                          class="w-full px-2 py-1 text-xs border rounded dark:bg-gray-900 dark:border-gray-600"
                          [ngClass]="{'border-red-400': row.fieldErrors['location']&&!row.skip,'border-gray-200':!row.fieldErrors['location']||row.skip}" />
                      </td>
                      <td class="px-1 py-1">
                        <input type="text" [(ngModel)]="row.data.quantity" (input)="validateRow(row)" [disabled]="row.skip"
                          class="w-full px-2 py-1 text-xs border rounded dark:bg-gray-900 dark:border-gray-600"
                          [ngClass]="{'border-red-400': row.fieldErrors['quantity']&&!row.skip,'border-gray-200':!row.fieldErrors['quantity']||row.skip}" />
                      </td>
                    ...
                    @if (!row.skip && row.status==='exists' && row.existingInventory) {
                      <tr class="bg-blue-50 dark:bg-blue-900/10">
                        <td colspan="11" class="px-4 py-1.5 text-xs text-blue-700">
                          {{ t('article_exists') }}: <strong>{{ row.existingInventory.sku }}</strong> en {{ row.existingInventory.location }} — {{ row.existingInventory.quantity }} uds
                        </td>
                      </tr>
                    }
                    @if (!row.skip && row.status==='similar' && row.similarInventory?.length) {
                      <tr class="bg-yellow-50 dark:bg-yellow-900/10 border-b border-yellow-100">
                        <td colspan="11" class="px-4 py-2">
                          <div class="flex items-center gap-3 text-xs">
                            <span class="text-yellow-700">⚠ Mismo SKU en otra ubicación:</span>
                            <div class="flex flex-wrap gap-2 flex-1">
                              @for (s of row.similarInventory; track s.id) {
                                <span class="px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded border border-yellow-200">{{ s.location }} ({{ s.quantity }} uds)</span>
                              }
                            </div>
      ...
      @if (isImporting) {
        <div class="px-6 py-3 bg-blue-50 dark:bg-blue-900/20 border-t border-blue-200">
          <div class="flex justify-between text-xs text-blue-700 mb-1.5">
            <span>{{ t('importing') || 'Importando...' }} {{ validRows.length }} {{ t('inventory_label') || 'ítems' }}</span>
        ...
        <div class="flex gap-3">
          ...
          @if (step === 3) {
            <button z-button zType="default" type="button"
              ...
              (click)="startImport()">
              {{ t('import') || 'Importar' }} {{ validRows.length > 0 ? validRows.length+' '+(t('inventory_label')||'ítems') : '' }}
            </button>
          }
        </div>
      </div>
...
export class InventoryImportDialogComponent {
  ...
  readonly data = inject<InventoryImportDialogData>(Z_MODAL_DATA);
  ...
  downloadTemplate(): void {
    ...
    fetch(`${environment.API.BASE}/inventory/import/template?lang=${lang}`, { headers })
      .then(r => r.blob()).then(blob => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a'); a.href = url; a.download = 'ImportInventory.xlsx';
    ...
    try {
      ...
      for (let i = 8; i < raw.length; i++) {
        ...
        const previewRow: PreviewRow = {
          originalRowNum: i+1, skip: sku.toUpperCase()==='SKU-0001',
          isExample: sku.toUpperCase()==='SKU-0001', status: 'new', fieldErrors: {},
          data: {
            sku, name: String(row[1]??'').trim(), description: String(row[2]??'').trim(),
            location: String(row[3]??'').trim(), quantity: String(row[4]??'').trim(),
            unit_price: String(row[5]??'').trim(),
            track_by_lot: normalizeBool(String(row[6]??'')),
            track_by_serial: normalizeBool(String(row[7]??'')),
            track_expiration: normalizeBool(String(row[8]??'')),
            min_quantity: String(row[9]??'').trim(), max_quantity: String(row[10]??'').trim(),
          },
        };
  ...
  private async validateWithBackend(): Promise<void> {
    ...
    try {
      ...
      if (token) headers['Authorization'] = `Bearer ${token}`;
      const res = await fetch(`${environment.API.BASE}/inventory/import/validate`, {method:'POST',headers,body:JSON.stringify(payload)});
      ...
      for (const row of this.rows) {
        ...
        row.existingInventory = result.existing_inventory;
        row.similarInventory = result.similar_inventory;
  ...
  validateRow(row: PreviewRow): void {
    ...
    if (!row.data.location) row.fieldErrors['location'] = 'Ubicación requerida';
    if (!row.data.quantity) row.fieldErrors['quantity'] = 'Cantidad requerida';
    else if (isNaN(Number(row.data.quantity))) row.fieldErrors['quantity'] = 'Debe ser número';
  ...
  async startImport(): Promise<void> {
    ...
    try {
      const res = await fetch(`${environment.API.BASE}/inventory/import/json`, {method:'POST',headers,body:JSON.stringify(payload)});
      const json = await res.json();
      clearInterval(anim!);
...
function normalizeBool(val: string): string {
