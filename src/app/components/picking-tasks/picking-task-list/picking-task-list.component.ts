...
export class PickingTaskListComponent {
  ...
  getPriorityBadge(priority: string): { variant: string; className: string; text: string } {
    const priorityConfig: Record<string, { variant: string; className: string; text: string }> = {
      normal: {
        variant: 'outline',
  ...
  onViewDetails(task: PickingTask): void {
    this.dialogService.create({
      zTitle: `${this.t('picking_task_label')} - ${task.task_id}`,
      zContent: PickingTaskDetailsContentComponent,
      zData: {
        task,
        getUserDisplayName: (uid: string | null) => this.getUserDisplayName(uid),
        getStatusBadge: (s: string) => this.getStatusBadge(s),
        getPriorityBadge: (p: string) => this.getPriorityBadge(p),
        onStatusUpdated: () => this.refresh.emit(),
      },
      zHideFooter: true,
