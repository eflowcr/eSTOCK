...
import {CreatePickingTaskRequest, PickingTask} from '@app/models/picking-task.model';
...
export class PickingTaskFormComponent implements OnInit {
  ...
  lotsWithQuantityPerItem: Array<
      {lot_number: string, quantity: number, expiration_date: string|null}[]> =
      [];
  ...
  // Available inventory for every item
  availableInventory: AvailableQuantity[] = [];

  constructor(
      private fb: FormBuilder, private pickingTaskService: PickingTaskService,
      private locationService: LocationService,
      private userService: UserService, private articleService: ArticleService,
      private inventoryService: InventoryService,
      private lotService: LotService, private serialService: SerialService,
  ...
  private isErrorResponse(response: any): boolean {
    ...
    if (response.result.message) {
      ...
      return message.includes('not enough') ||
          ...
          message.includes('unable') || message.includes('invalid') ||
          message.includes('denied') || message.includes('exceeded');
    }
  ...
  filterOperators(): void {
    ...
    this.filteredOperators = this.users.filter(
        user => (user.first_name || '').toLowerCase().includes(term) ||
  ...
  async loadData(): Promise<void> {
    try {
      ...
      const [locationResponse, userResponse, inventoryResponse] =
          await Promise.all([
            this.locationService.getAll(), this.userService.getAll(),
      ...
      if (inventoryResponse.result.success) {
        ...
        this.inventoryData.forEach(item => {
          if (item.quantity > 0) {
            ...
            const location =
                this.locations.find(loc => loc.location_code === item.location);
            if (location &&
                !currentLocations.find(
                    loc => loc.location_code === item.location)) {
    ...
    if (this.task.items && this.task.items.length > 0) {
      this.task.items.forEach((item, i) => {
        ...
        if (item.lots && Array.isArray(item.lots)) {
          ...
          if (oldLotNumbers && oldLotNumbers.length > 0) {
            oldLotNumbers.forEach((lotNumber: string) => {
              ...
              this.lotsWithQuantityPerItem[i].push(
                  {lot_number: lotNumber, quantity: 1, expiration_date: null});
            });
          }
        }
        ...
        this.skuSearchTerms[i] =
            article ? `${article.sku} - ${article.name}` : item.sku;

        const loc = this.locations.find(l => l.location_code === item.location);
        this.locationSearchTerms[i] =
            loc ? `${loc.location_code} - ${loc.description}` : item.location;
  ...
  addItem(): void {
    if (!this.canAddItem()) {
      this.alertService.warning(
          this.t('complete_current_item_before_adding_new'), this.t('warning'));
      return;
    }
  ...
  async onSubmit(): Promise<void> {
    if (this.form.invalid || !this.isFormComplete()) {
      ...
      this.alertService.error(
          this.t('please_complete_required_fields'), this.t('error'));
      return;
    }
    ...
    try {
      ...
      const taskData: any = {
        ...
        items: formValue.items.map((item: any, index: number) => {
          ...
          if (item.lot_numbers) {
            const lotNumbers = item.lot_numbers.split(',')
                                   .map((s: string) => s.trim())
                                   .filter((s: string) => s);
            const lots: any[] = [];
      ...
      if (this.task) {
        const response =
            await this.pickingTaskService.update(this.task.id, taskData);

        if (this.isErrorResponse(response)) {
          this.alertService.error(
              response.result.message ||
                  this.t('failed_to_update_picking_task'),
              this.t('error'));
        } else {
          this.alertService.success(
              this.t('picking_task_updated_successfully'), this.t('success'));
          this.success.emit();
        }
      } else {
        ...
        if (this.isErrorResponse(response)) {
          this.alertService.error(
              response.result.message ||
                  this.t('failed_to_create_picking_task'),
              this.t('error'));
        } else {
          this.alertService.success(
              this.t('picking_task_created_successfully'), this.t('success'));
          this.success.emit();
        }
      }
    } catch (error) {
      this.alertService.error(this.t('error_saving_task'), this.t('error'));
    } finally {
      this.loadingService.hide();
    }
  }
  ...
  ensureComboboxState(index: number): void {
    if (this.skuSearchTerms[index] === undefined)
      this.skuSearchTerms[index] = '';
    if (this.locationSearchTerms[index] === undefined)
      this.locationSearchTerms[index] = '';
    if (this.showSkuDropdown[index] === undefined)
      this.showSkuDropdown[index] = false;
    if (this.showLocationDropdown[index] === undefined)
  ...
  filterArticlesForItem(index: number): void {
    ...
    this.filteredArticlesPerItem[index] = this.articles.filter(
        a => (a.sku || '').toLowerCase().includes(term) ||
  ...
  filterLocationsForItem(index: number): void {
    ...
    // Get available locations for the selected SKU (only locations with
    // inventory)
    const availableLocations =
        selectedSku ? (this.articleLocationMap.get(selectedSku) || []) : [];
    ...
    this.filteredLocationsPerItem[index] = availableLocations.filter(
        l => (l.location_code || '').toLowerCase().includes(term) ||
  ...
  onSkuSelected(index: number, article: Article): void {
    ...
    if (availableLocations.length === 1) {
      ...
      this.itemsArray.at(index)
          .get('location')
          ?.setValue(autoLocation.location_code);
      this.locationSearchTerms[index] =
          `${autoLocation.location_code} - ${autoLocation.description || ''}`;
    } else {
      this.itemsArray.at(index).get('location')?.setValue('');
      this.locationSearchTerms[index] = '';
    }
  ...
  isLocationValid(index: number): boolean {
    ...
    return !!formValue &&
        this.locations.some(l => l.location_code === formValue);
  }
  ...
  private async loadTrackingOptionsForItem(index: number, sku: string):
      Promise<void> {
    // First try to get article from articles array
  ...
  filterLotsForItem(index: number): void {
    ...
    this.filteredLotObjectsPerItem[index] = term ?
        lotObjects.filter(
            lot => (lot.lot_number || '').toLowerCase().includes(term)) :
        [...lotObjects];
    // Keep backward compatibility
    const options = this.availableLotsPerItem[index] || [];
    this.filteredLotsPerItem[index] = term ?
  ...
  async loadExistingLotData(
      itemIndex: number, sku: string, lotNumbers: string[]): Promise<void> {
    try {
      ...
      if (response.result.success && response.data) {
        ...
        for (const lotNumber of lotNumbers) {
          const lotData =
              response.data.find(lot => lot.lot_number === lotNumber);
        ...
        this.lotsWithQuantityPerItem[itemIndex] = lotNumbers.map(
            lotNumber =>
                ({lot_number: lotNumber, quantity: 1, expiration_date: null}));
      }
    } catch (error) {
  ...
  onRequiredQuantityChange(index: number): void {
    ...
    var quantity = this.getAvailableInventoryForItems(
        this.getIndexSKU(index), this.getIndexLocation(index));

    if (quantity < this.requiredQuantities[index]) {
      // Set item field error
      this.alertService.warning(
          this.t('not_enough_inventory_available'), this.t('warning'));
      this.requiredQuantities[index] = quantity;
    }
  }
  ...
  onLotSelected(index: number, lotNumber: string): void {
    ...
    if (current.includes(lotNumber)) {
      ...
      this.alertService.warning(
          this.t('lot_selection_limit_reached'), this.t('warning'));
    }
    this.showLotDropdown[index] = false;
  }
  ...
  onSerialSelected(index: number, serialNumber: string): void {
    ...
    if (current.includes(serialNumber)) {
      ...
      this.alertService.warning(
          this.t('serial_selection_limit_reached'), this.t('warning'));
    }
    this.showSerialDropdown[index] = false;
  }
  ...
  handleLotEnter(index: number): void {
    ...
    if (searchTerm) {
      ...
      const exactMatch =
          filtered.find(lot => lot.toLowerCase() === searchTerm.toLowerCase());
  ...
  handleSerialEnter(index: number): void {
    ...
    if (searchTerm) {
      ...
      const exactMatch = filtered.find(
          serial => serial.toLowerCase() === searchTerm.toLowerCase());
  ...
  addManualSerial(index: number, serialNumber: string): void {
    ...
    if (!current.includes(serialNumber.trim()) &&
        current.length < requiredQty) {
  ...
  addLotWithQuantity(index: number): void {
    ...
    if (this.lotsWithQuantityPerItem[index].some(
            lot => lot.lot_number === lotNumber)) {
      this.alertService.warning(
          this.t('lot_already_exists'), this.t('warning'));
      return;
    }
    ...
    if (currentTotal + quantity > requiredQty) {
      ...
      this.alertService.warning(
          this.t('lot_quantity_exceeds_total') ||
              `Solo quedan ${remaining} unidades disponibles`,
          this.t('warning'));
      return;
    }
    ...
    const lotNumbers =
        this.lotsWithQuantityPerItem[index].map(lot => lot.lot_number);
    this.itemsArray.at(index)
        .get('lot_numbers')
        ?.setValue(lotNumbers.join(', '));
  ...
  getTotalLotQuantityForItem(index: number): number {
    return this.lotsWithQuantityPerItem[index]?.reduce(
  ...
  isLotExpiringSoon(expirationDate: string|null): boolean {
    ...
    return daysUntilExpiry <= 30 &&
        daysUntilExpiry > 0;  // Expires within 30 days
  }
  ...
  isFormComplete(): boolean {
    for (let i = 0; i < this.itemsArray.length; i++) {
      ...
      if (article?.track_by_lot) {
        ...
        if (this.lotsWithQuantityPerItem[i] &&
            this.lotsWithQuantityPerItem[i].length > 0) {
          // Quantity-based lots: allow partial quantities (not exceeding
          // required)
  ...
  // Get available inventory for every item in itemsArray by SKU and location
  getAvailableInventoryForItems(sku: string, location: string): number {
    const inventoryItem = this.inventoryData.find(
        item => item.sku === sku && item.location === location);
    return inventoryItem ? inventoryItem.quantity || 0 : 0;
  }

  getIndexSKU(index: number): string {
    return this.itemsArray.at(index).get('sku')?.value || '';
  }

  getIndexLocation(index: number): string {
    return this.itemsArray.at(index).get('location')?.value || '';
  }
}
